    ...
    <div class="space-y-6">
        ...
        <div class="ui-card p-6">
            <div class="flex items-start justify-between gap-3">
                <div>
                    ...
                    <p class="mt-1 text-sm text-slate-600">E-postanı güncelle, cihazlarını görüntüle.</p>
                </div>
                <span class="ui-badge">Rol: ÖĞRENCİ</span>
            </div>
        </div>
        <div class="ui-card p-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
            ...
            <div class="lg:col-span-7 space-y-4">
                ...
                <div class="ui-card p-6">
                    ...
                    <form method="POST" action="{{ route('profil.guncelle') }}" class="mt-5 space-y-4">
                        ...
                        <div>
                            ...
                            {{-- Eğer backend hala ad_soyad zorunlu validate ediyorsa: --}}
                            <input type="hidden" name="ad_soyad" value="{{ $u->ad_soyad }}">
                        </div>

                        <div>
                            <label class="text-sm font-semibold text-slate-800">Öğrenci No</label>
                            <input value="{{ $u->ogrenci_no }}" readonly tabindex="-1"
                                class="ui-input mt-1 bg-slate-50/70 text-slate-700 cursor-not-allowed">
                            <input type="hidden" name="ogrenci_no" value="{{ $u->ogrenci_no }}">
                        </div>
                ...
                {{-- Öğrenci hızlı aksiyon --}}
                <div class="ui-card p-6">
                    <div class="flex items-center justify-between">
                        <h3 class="text-lg font-extrabold tracking-tight text-slate-900">Hızlı İşlemler</h3>
                        <x-heroicon-o-bolt class="h-6 w-6 text-slate-600" />
                    </div>

                    <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2">
                        <div class="text-sm text-slate-600">İleride: Ders kayıt & katılım geçmişi buraya gelecek.</div>
                    </div>
                </div>
